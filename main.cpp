...
static unsigned int		g_inDebug = true;
static unsigned int		g_FrameRate = true;
static unsigned int		g_50Hz = false;
...
inline static void DebugInfo()
{
	...
	//sprintf(textout, "ALIENIGENAS! Bispoo (c) 2018");
...
static void app_initialise()
{
    ...
    #else
     sdlWindow = SDL_CreateWindow(NAME, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 0, 0, SDL_WINDOW_FULLSCREEN_DESKTOP);
    #endif
 ...
 #else
    sdlRenderer = SDL_CreateRenderer(sdlWindow, -1, SDL_RENDERER_SOFTWARE | SDL_RENDERER_TARGETTEXTURE) ;
